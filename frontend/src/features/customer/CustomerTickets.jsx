    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-400">
            Track your support history and open each conversation.
          </p>
        </div>

        <button
          onClick={() => navigate("/customer/new-ticket")}
          className="cursor-pointer rounded-2xl bg-indigo-500 px-5 py-3 font-semibold text-white transition hover:-translate-y-0.5 hover:bg-indigo-600"
        >
          New Ticket
        </button>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <input
          ...
          className="rounded-2xl border border-white/10 bg-slate-900/60 px-4 py-3 text-white outline-none"
        />
      ...
      {tickets.length === 0 ? (
        <EmptyState
          ...
          description="Create your first ticket to start the support process."
          action={
            <button
              onClick={() => navigate("/customer/new-ticket")}
              className="cursor-pointer rounded-2xl bg-indigo-500 px-5 py-3 font-semibold text-white"
            >
              Create Ticket
            </button>
          }
